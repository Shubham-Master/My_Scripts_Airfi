...
PRODUCT_ID_FILE="/media/product_id.txt"
...
while IFS= read -r product_id; do
  # Read IP addresses from the ARP command output and iterate over them
  arp | awk '/[0-9a-f:]{17}/ {print $2}' | sed 's/[()]//g' | while read -r ip; do
