...
manifest_ids=$(curl -s http://admin:V3ryS3cur3@localhost:5984/installed-manifests-g9/_all_docs | grep drmkey | grep -o '"id":"[^"]*"' | sed 's/"id":"\([^"]*\)"/\1/' | sed 's/\//%2f/g')
...
echo "$manifest_ids" | while read -r man; do
    ...
    rev=$(curl -s http://admin:V3ryS3cur3@localhost:5984/installed-manifests-g9/$man | grep -o '"_rev":"[^"]*"' | sed 's/"_rev":"\([^"]*\)"/\1/')

    # Check if the revision was retrieved successfully
    if [ -n "$rev" ]; then
        # Attempt to delete the document with the retrieved revision
        delete_response=$(curl -s -X DELETE "http://admin:V3ryS3cur3@localhost:5984/installed-manifests-g9/$man?rev=$rev")
